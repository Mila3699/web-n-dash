...
const Training = () => {
  const { getBlockContent } = useContent('training');
  
  const forWhoPoints = [
    getBlockContent('training-for-who-1') || "вы хотите глубоко изучить энергию Кундалини и стать энерготерапевтом-фасилитатором с осознанным подходом и поддержкой сообщества",
    getBlockContent('training-for-who-2') || "вы готовы к серьёзным внутренним изменениям и желаете развивать эмпатию и энергетическую проводимость",
    getBlockContent('training-for-who-3') || "вы ищите экологичный и безопасный путь работы с энергией, без эзотерических ритуалов и с научным подходом",
    getBlockContent('training-for-who-4') || "вы хотите работать с энергией профессионально и получать стабильный доход, обучившись профессии будущего",
    getBlockContent('training-for-who-5') || "вы готовы пройти подготовку и адаптацию, готовы к новым вибрациям и большим изменениям в своей жизни",
    getBlockContent('training-for-who-6') || "вы фасилитатор других систем и желаете повысить квалификацию и дорасшириться в работе с энергией Кундалини"
  ];

  const methodElements = [
    {
      icon: Sparkles,
      title: getBlockContent('training-method-1-title') || "Активация кундалини",
      description: getBlockContent('training-method-1-desc') || "Пробуждение внутреннего ресурса, энергии жизни, которая раскрывает потенциал и запускает глубинные трансформации. Раскрытие сверхспособностей - яснознание, ясновидение и др"
    },
    {
      icon: Eye,
      title: getBlockContent('training-method-2-title') || "Чтение поля",
      description: getBlockContent('training-method-2-desc') || "Работа с невидимыми слоями информации. Вы начинаете лучше чувствовать себя и других, понимать корни проблем и видеть больше, чем разум способен объяснить."
    },
    {
      icon: Wind,
      title: getBlockContent('training-method-3-title') || "Очищение поля",
      description: getBlockContent('training-method-3-desc') || "Снятие энергетических и эмоциональных блоков, зажимов, устаревших программ, которые мешают двигаться вперёд. Уходят обиды, страхи, боль, сомнения."
    },
    {
      icon: Heart,
      title: getBlockContent('training-method-4-title') || "Восстановление энергосистемы",
      description: getBlockContent('training-method-4-desc') || "Балансировка всех уровней: от тела до сознания. Это даёт ясность, лёгкость, новые силы и ощущение внутренней опоры, уверенность. Открывается сердце, появляется внутренняя гармония"
    }
  ];
  ...
  const benefits = [
    {
      icon: Award,
      title: getBlockContent('training-benefit-1') || "Сертификат международного образца",
    },
    {
      icon: Users,
      title: getBlockContent('training-benefit-2') || "Доступ в закрытое сообщество мастеров ERA",
    },
    {
      icon: Video,
      title: getBlockContent('training-benefit-3') || "Навыки проведения онлайн и офлайн сессий",
    },
    {
      icon: BookOpen,
      title: getBlockContent('training-benefit-4') || "Глубокая личная трансформация",
    },
    {
      icon: Globe,
      title: getBlockContent('training-benefit-5') || "Собственная страница на нашем сайте и поток клиентов",
    },
  ];
